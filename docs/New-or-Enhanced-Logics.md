...
### Power drain
- It is now possible for buildings to provide both power output and power drain by specyfing `PowerDrain`.

In `rulesmd.ini`:
```ini
[SOMEBUILDING]                      ; BuildingType
PowerDrain=0                        ; integer
```

### Power plant enhancer

- When it exists, it can increase the power amount generated by the power plants or power amount drained by other structures.
...
PowerPlantDrainEnhancer.PowerPlants=    ; list of BuildingTypes
PowerPlantDrainEnhancer.Amount=0        ; integer
